...
private:
	...
	template<MixMode mixMode>
	int convertForMixMode(AudioStream &input, byte *outBuffer, uint outBytesPerSample, st_size_t numSamples, st_volume_t volL, st_volume_t volR);
...
template<bool inStereo, bool outStereo, bool reverseStereo>
template<MixMode mixMode>
int RateConverter_Impl<inStereo, outStereo, reverseStereo>::convertForMixMode(AudioStream &input, byte *outBuffer, uint outBytesPerSample, st_size_t numSamples, st_volume_t volL, st_volume_t volR) {
	if (outBytesPerSample == sizeof(int32)) {
		return convertForType<int32, mixMode>(input, outBuffer, numSamples, volL, volR);
	} else {
		return convertForType<int16, mixMode>(input, outBuffer, numSamples, volL, volR);
	}
}
...
int RateConverter_Impl<inStereo, outStereo, reverseStereo>::convert(AudioStream &input, byte *outBuffer, uint outBytesPerSample, st_size_t numSamples, st_volume_t volL, st_volume_t volR, MixMode mixMode) {
	if (mixMode == MIX_ASSIGN)
		return convertForMixMode<MIX_ASSIGN>(input, outBuffer, outBytesPerSample, numSamples, volL, volR);
	else if (mixMode == MIX_ADD)
		return convertForMixMode<MIX_ADD>(input, outBuffer, outBytesPerSample, numSamples, volL, volR);
	else
		return convertForMixMode<MIX_CLAMPED_ADD>(input, outBuffer, outBytesPerSample, numSamples, volL, volR);
}
