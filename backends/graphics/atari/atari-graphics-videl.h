...
private:
	...
	void copySurfaceToSurface(const Graphics::Surface &srcSurface, Graphics::Surface &dstSurface) const override {
		asm_c2p1x1_8(
			(const char*)srcSurface.getPixels(),
			(const char*)srcSurface.getBasePtr(srcSurface.w, srcSurface.h-1),
			(char*)dstSurface.getPixels());
	}
	...
	void copyRectToSurface(const Graphics::Surface &srcSurface, int destX, int destY,
		...
		asm_c2p1x1_8_rect(
			(const char*)srcSurface.getBasePtr(subRect.left, subRect.top),
			(const char*)srcSurface.getBasePtr(subRect.right, subRect.bottom-1),
			subRect.width(),
			srcSurface.pitch,
			(char*)dstSurface.getBasePtr(destX, destY),
			dstSurface.pitch);
	}
